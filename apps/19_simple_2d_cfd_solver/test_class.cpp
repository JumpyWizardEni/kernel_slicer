...
void Solver::perform(float *out_density) {
    ...
    for (int k = 0; k < STEPS_NUM; k++) {
        kernel2D_Diffuse(size - 2, size - 2, 1, vx.data(), vx0.data(), visc);
        set_bounds((int) size, 1, vx.data());

    }
    swap(vy0, vy);
    for (int k = 0; k < STEPS_NUM; k++) {
        kernel2D_Diffuse(size - 2, size - 2, 2, vy.data(), vy0.data(), visc);
        set_bounds((int) size, 2, vy.data());
    }
    ...
    set_bounds(size, 1, vx.data());

    kernel2D_Advect(size - 2, size - 2, dt, 2, vy.data(), vy0.data(), vx0.data(), vy0.data());
    set_bounds(size, 2, vy.data());


    kernel2D_Project_1(size - 2, size - 2, vx.data(), vy.data(), vx0.data(), vy0.data());
    set_bounds(size, 0, vy0.data());
    set_bounds(size, 0, vx0.data());
    for (int k = 0; k < STEPS_NUM; k++) {
        kernel2D_Project_2(size - 2, size - 2, vx.data(), vy.data(), vx0.data(), vy0.data());
        set_bounds(size, 0, vx0.data());
    ...
    for (int k = 0; k < STEPS_NUM; k++) {
        kernel2D_Diffuse(size - 2, size - 2, 0, density.data(), density_prev.data(), diff);
        set_bounds(size, 0, density.data());
    }
    swap(density_prev, density);
